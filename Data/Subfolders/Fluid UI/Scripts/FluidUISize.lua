-- FluidUISize.lua
-- Handles dynamic scaling of UI Elements
...
function Update()
	...
	UIControl.width = math.floor((parentSize.x * Percent.x) + Pixels.x)
	UIControl.height = math.floor((parentSize.y * Percent.y) + Pixels.y)

	InternalAPI.Print(string.format("Updated Size of %s", UIControl.name))
end
